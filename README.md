...
- `HistoryContextBuilder` reads transcript messages after that watermark, capped at `MAX_CONTEXT_HISTORY_MESSAGES`, and appends an attachment manifest line to each past user message that had one (see **Attachments** below) — one query for every user message in the window, not one per message.
...
**Attachments** (`app/services/chat_service.py`, `app/agent/context/history_context_builder.py`, `app/utils/attachment_manifest.py`, `app/utils/images.py`, `app/plugins/attachments/`). A user can attach images and documents to a message (`POST /conversations/{id}/uploads`, described in the API table above); this section is how the model finds out about them, which is a separate concern from storing them. Every attachment gets one manifest line — `format_attachment_manifest_line` — of the shape `[attachment id=<uuid> name=<filename> type=<content_type> size=<n>] <note>`, and it is the single place that line is composed, so the turn an attachment arrives on and every later replay of it through history describe it identically.

The turn an attachment is uploaded on is the only turn that can carry its bytes. `ChatService._build_human_message` turns the plain-text `HumanMessage` LangChain normally sees into content blocks — the user's text, a manifest line per attachment, and (only when `LLM_SUPPORTS_VISION` is on) each image, downscaled to at most 2048px on its long side by `app/utils/images.py` and base64-encoded — whenever the message has any attachments at all; a message with none is still built exactly as before. `LLM_SUPPORTS_VISION` defaults to `false`, since not every model behind `LLM_PROVIDER` understands an image content block; with it off, an image's manifest line says outright that the model cannot see it, rather than silently including nothing. Reading and downscaling an image can itself fail (a corrupt upload, an edge-case format) — that failure never fails the turn, only that one image, and the manifest line says the image could not be prepared rather than claiming an image that is not actually there.

Every later turn, `HistoryContextBuilder` appends the same manifest lines as plain text to each past user message — never bytes, and the manifest line says so explicitly ("visible to you only on the turn it was attached"), so the model is never left assuming it can still see an image from three turns ago. The persisted `messages.content` row is never touched by any of this: it stays exactly what the user typed, so the manifest is invisible in the UI and exists purely as model input, assembled fresh on each turn.

For a non-image attachment — pdf, docx, or plain text — the manifest line's note is simply an instruction to call `read_attachment`. That tool (`app/plugins/attachments/tools.py`) verifies the requested id belongs to the conversation in `config["configurable"]["thread_id"]` before touching it, exactly like `documents/tools.py`'s identity check, and returns a not-found message rather than an error for an id that does not resolve — the id is model-supplied and the model can pass anything. Extraction (`pypdf` for pdf, `python-docx` for docx, UTF-8 decoding for text) runs in `asyncio.to_thread`, since it is synchronous work that must not block the event loop serving SSE, and a failure to extract is reported back to the model as text rather than raised — an unreadable attachment is data the user supplied, not an application error. The extracted text is paginated at roughly 8,000 characters — a page size chosen for the tool's own response budget, unrelated to the file's native page count — and the response states which page came back and how many exist, so the model can call again for more. An image's `read_attachment` response is metadata only, with the same "visible only on the turn it was attached" note as its manifest line.
...
│   │   │   └── tools.py                          # Ownership check, pypdf/python-docx/UTF-8 extraction, ~8,000-char pagination
...
│       ├── detect.py                             # Upload type sniffing by magic bytes: png/jpeg/pdf/docx/utf-8 text
│       ├── images.py                             # downscale_image() — the vision content-block resize
│       └── attachment_manifest.py                # format_attachment_manifest_line() — one line, shared by the current turn and history
