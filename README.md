...
| `RECYCLING_VISION_MODEL` | *(empty)* | Overrides which model `/recycle scan_image` and `/recycle build_catalog` call for vision. Unset means the recycling plugin uses the app's own `MODEL_NAME` and credentials — see **Tool plugins** below |
...
Three plugins use it today. `attachments` contributes a short note — attachment contents do not survive a turn, so re-read before answering — which is exactly the kind of lifecycle fact a tool description cannot carry, because the model needs it *before* choosing to call anything. `recruitment` contributes something much larger: the interview steps, the pre-generation checks, the two document tools, and the blank forms, all of which used to sit in `app/prompts/anna/system_prompt.txt`. Moving them changed no totals — the same text is sent either way — but it put the knowledge where the capability is, so the persona file now describes only who Anna is and how she behaves, and a deployment that names `recruitment` in `EXCLUDED_TOOL_PLUGINS` no longer tells the model to call `generate_rirekisho`. `recycling` contributes the fact that `/recycle scan_video` and `scan_image` exist — the model cannot run a command, so without this it could not tell a user how to get a scan — plus the video recording rules (landscape, slow sweep, lights on), which describe this scanner and so must disappear with it. `clock` contributes nothing: its one tool is self-describing. Both still have a `prompts.py` (recycling's is `pipeline/prompts.py`, kept inside that subpackage because importing from the plugin root would re-enter the partially-initialised package) — a plugin contributing nothing to the system prompt still has runtime text worth keeping out of its logic.
...
│   │   ├── contracts.py                          # ToolContext (+ LLMAccess), ToolPlugin, PluginCommand/CommandContext — the contract every plugin is built against
│   │   ├── loader.py                             # Discovers plugin folders, calls each factory + command_factory, collects each system_prompt, enforces unique tool names/namespaces + token budgets
│   │   ├── clock/
...
│   │   ├── attachments/
│   │   │   ├── __init__.py                       # PLUGIN — exposes read_attachment, injects plugin_prompt.txt
│   │   │   ├── plugin_prompt.txt                 # Injected into the system prompt while this plugin is loaded: attachment contents do not survive a turn
│   │   │   ├── prompts.py                        # Everything the plugin says at runtime: tool description, remedies, per-outcome messages
│   │   │   └── tools.py                          # Ownership check, pypdf/text extraction, scanned-PDF warnings, ~8,000-char pagination
│   │   ├── recycling/                            # /recycle — scan_image, scan_video [frames], build_catalog, show_catalog; plugin_prompt.txt = commands + recording rules. Named in the deployment's EXCLUDED_TOOL_PLUGINS.
│   │   │   ├── __init__.py                       # PLUGIN — builds the one openai client (ToolContext.llm), registers /recycle, no tools
│   │   │   ├── commands.py                       # Subcommand dispatch, Markdown rendering, attaches the full JSON result for later read_attachment
│   │   │   ├── runner.py                         # Async glue: fires consensus runs concurrently via asyncio.to_thread so a scan never blocks the event loop
│   │   │   ├── catalog.json                      # The plugin's own catalog copy (phase 4 moves this to Postgres)
...
│       ├── filenames.py                          # clean_filename() — display-safe upload names (no control/bidi chars, no brackets)
