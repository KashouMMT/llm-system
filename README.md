...
| `ENABLED_TOOL_PLUGINS` | *(empty)* | Comma-separated allowlist of plugin folder names under `app/plugins/`. Empty means load every plugin folder that is present — **on the deploy branch this must explicitly name every plugin the deployment wants**, e.g. `clock,documents,attachments`, since it is an allowlist: forgetting one silently disables it, and `recycling` must stay off the job-application deployment entirely |
...
│   │   ├── loader.py                             # Discovers plugin folders, calls each factory + command_factory, enforces unique tool names/namespaces + token budget
│   │   ├── clock/
...
│   │   ├── attachments/
│   │   │   ├── __init__.py                       # PLUGIN — exposes read_attachment
│   │   │   └── tools.py                          # Ownership check, pypdf/text extraction, scanned-PDF warnings, ~8,000-char pagination
│   │   ├── recycling/                            # /recycle — scan_image, scan_video (stub), build_catalog, show_catalog. Excluded from the deploy branch's ENABLED_TOOL_PLUGINS.
