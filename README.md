...
Every route except `POST /auth/login` and `POST /auth/register` requires the session cookie. Conversation, message, and event routes are scoped to the authenticated user; a conversation owned by someone else is reported as `404`.
...
| `POST` | `/auth/login` | Body: `{"email": str, "password": str}`. On success, sets the `HttpOnly` session cookie and returns `{id, email, role}`; `401` on bad credentials |
| `POST` | `/auth/logout` | Deletes the current session and clears the cookie. `204` |
| `GET` | `/auth/me` | The current user as `{id, email, role}`, or `401` |
