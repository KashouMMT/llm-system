...
| `UPLOAD_DAILY_BYTES_PER_USER` | `209715200` (200 MB) | Cap on how many bytes one user may upload in a rolling 24 hours, across every conversation. `POST /conversations/{id}/uploads` rejects with `429` once this would be exceeded. Computed by summing `files.size_bytes` where `origin='uploaded'` for that user in the last day (`idx_files_user_uploaded_created` backs the query); generated documents don't count against it |
...
Two plugins use it today. `attachments` contributes a short note — attachment contents do not survive a turn, so re-read before answering — which is exactly the kind of lifecycle fact a tool description cannot carry, because the model needs it *before* choosing to call anything. `recruitment` contributes something much larger: the interview steps, the pre-generation checks, the two document tools, and the blank forms, all of which used to sit in `app/prompts/anna/system_prompt.txt`. Moving them changed no totals — the same text is sent either way — but it put the knowledge where the capability is, so the persona file now describes only who Anna is and how she behaves, and a deployment that names `recruitment` in `EXCLUDED_TOOL_PLUGINS` no longer tells the model to call `generate_rirekisho`. `clock` and `recycling` contribute nothing: `clock`'s one tool is self-describing, and `recycling` exposes no tools at all, since a slash command never reaches the agent. Both still have a `prompts.py` (recycling's is `pipeline/prompts.py`, kept inside that subpackage because importing from the plugin root would re-enter the partially-initialised package) — a plugin contributing nothing to the system prompt still has runtime text worth keeping out of its logic.
...
**Supported types** are decided by `app/utils/detect.py` from the bytes, never from the extension: png, jpeg and webp images; pdf; and plain text in UTF-8 or Shift_JIS (`cp932`, which is what Windows and Japanese Excel actually write), which covers `.txt`, `.md`, `.csv` and `.json` alike. Office formats are deliberately excluded — a `.docx` or `.xlsx` is mostly tables and layout, and extracting only its paragraphs hands the model half a document without saying so. A row of a type that is no longer accepted is described to the model as unreadable, with an instruction to ask for a PDF or a screenshot.
...
| `POST` | `/conversations/{id}/uploads?filename={name}` | Store one attachment, before it is attached to any message. Raw body (not multipart), streamed and capped at `UPLOAD_MAX_BYTES` — `413` if exceeded. `filename` is a percent-encoded query parameter, since it may be Japanese and headers are latin-1. `Content-Type` is never trusted: the actual type is read from the bytes (png/jpeg/webp/pdf, or UTF-8/Shift_JIS text; anything else, or an image that fails to decode, is `415`). The filename is stored cleaned (see **Attachments**). Returns `201` with `{id, filename, content_type, size_bytes}` |
...
│   │   ├── recycling/                            # /recycle — scan_image, scan_video (stub), build_catalog, show_catalog. Named in the deployment's EXCLUDED_TOOL_PLUGINS.
...
│       ├── detect.py                             # Upload type sniffing by bytes: png/jpeg/webp/pdf, UTF-8/Shift_JIS text; decode_text()
