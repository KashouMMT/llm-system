...
| `SYSTEM_PROMPT` | `default` | Name of a folder under `app/prompts/` holding the prompt set (`system_prompt.txt` + the two summary prompts, plus an optional `first_message.txt`); the whole set falls back to `app/prompts/default/` if any of the three required files is missing or empty |
