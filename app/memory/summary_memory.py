...
class SummaryMemory:
    def __init__(self, llm, token_threshold: int = SUMMARY_TOKEN_THRESHOLD):
        self.llm = llm
    ...
    def generate_chunk_summary(self, rows):
    ...
    def update_master_summary(self, current_summary: str, chunk_summary: str):
        prompt = SUMMARY_MERGE_PROMPT.format(
            current_summary=current_summary, chunk_summary=chunk_summary
        )
        response = self.llm.invoke([HumanMessage(content=prompt)])
    ...
    def summarize(self, session_id: str):
        ...
        logger.info(
            f"Conversation summarized | session={session_id} messages={len(rows)}"
        )

    def trigger_if_needed(self, session_id: str):
        if not self.should_summarize(session_id):
            return

        self.summarize(session_id)

    def should_summarize(self, session_id: str):
        rows = self.get_unsummarized_messages(session_id)
