...
Run it from the project root and open the result in Word before committing:
    ...
    python -m app.plugins.recruitment.scripts.make_shokumu_template \
        "documentation/other/T【職務経歴書】.docx" \
        app/plugins/recruitment/templates/shokumu_keirekisho.docx
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit(
            "usage: python -m app.plugins.recruitment.scripts.make_shokumu_template "
            "<source.docx> "
            "<destination.docx>"
        )
